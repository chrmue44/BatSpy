...
int initInfoPan(cPanel* pan, tCoord lf) {
  ...
  err |= pan->addNumItem(&devStatus.cpuAudioAvg, 140,  30,            35, lf, false);
  err |= pan->addTextItem(401,                   180,  30,            80, lf);
  err |= pan->addTextItem(405,                     3,  30      + lf,  80, lf);
  err |= pan->addNumItem(&devStatus.cpuAudioMax, 140,  30      + lf,  35, lf, false);
  ...
  err |= pan->addNumItem(&devStatus.peakVal,     140,  30 +  3 * lf,  35, lf, false);
  ...
  err |= pan->addNumItem(&devStatus.temperature, 140,  30 + 10 * lf,  40, lf, false);
