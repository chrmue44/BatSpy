...
void  cAudio::setSampleRate(enSampleRate sr)
{
  ...
  if(m_old.sampleRate != sr)
  {
    ...
    m_old.sampleRate = sr;
    Serial.printf("factors c0:%i  c1:%i   c2:%i\n",c0, c1, c2);
    Serial.printf("setSampleRate(%d)\n",m_sampleRate);
  }
}
...
{
  bool retVal = false;
  if(m_old.sampleRate != devPars.sampleRate.get())
  {
...
void cAudio::setup()
{
  ...
  if(isSetupNeeded())
  {
    ...
    DPRINTF4("      mix freq: %f  setup freq: %f\n", freq, freq * AUDIO_SAMPLE_RATE_EXACT / m_sampleRate);
...
void cAudio::setOscFrequency(float freq)
{
  ...
  m_sineHet.frequency(freq * AUDIO_SAMPLE_RATE_EXACT / m_sampleRate );
