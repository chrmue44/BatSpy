    ...
    if (p != NULL)
    {
      ...
      if(clDebugLog::checkLogSource(Src))
      {
        ...
        snprintf(Buf, sizeof(Buf),
                 ...
                 m_ChunkCount, ClassName, (unsigned int)(unsigned long)(p), size,pPool->getChunkSize());
      ...
      if(p != NULL)
      {
        ...
        snprintf(Buf, sizeof(Buf),
                 "HEAP   : Chunks %ui; new %s; Addr %ui; reqSize %ul",
                  m_HeapTab.size(), ClassName, (unsigned int)(unsigned long)(p), size);
  ...
  if(found)
  {
    ...
    if(RetVal == 0)
    {
      ...
      if(clDebugLog::checkLogSource(Src))
      {
        ...
        m_ChunkCount, ClassName, (unsigned int)(unsigned long)(p), (*i)->getChunkSize());
        clDebugLog::add(Src,Buf);
      }
    }
  }
    ...
    if(p != NULL)
    {
      ...
      if(Iter != m_HeapTab.end())
      {
        ...
        if(clDebugLog::checkLogSource(DBGSRC_MEMDYNALLOC))
        {
          ...
          snprintf(Buf, sizeof(Buf),
                 ...
                 m_HeapTab.size(), ClassName, (unsigned int)(unsigned long)(p));
          clDebugLog::add(DBGSRC_MEMDYNALLOC,Buf);
        }
      }
      else
      {
        if(clDebugLog::checkLogSource(DBGSRC_MEMFIXALLOC))
        {
          ...
          ClassName, (unsigned int)(unsigned long)(p));
